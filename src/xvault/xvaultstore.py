...
class XVaultStore():
    ...
    def __init__(self, dbname, password = None, create = False):
        # get path
        self._name = dbname
        (self._path, self._path_config, self._git_project) = XVaultStore._get_store_path(dbname)
        # validate
    ...
    @staticmethod
    def get_db_names():
        (folder, _, _) = XVaultStore._get_store_path("*")
    ...
    def delete_db(dbname: str) -> bool:
        (path, _, _) = XVaultStore._get_store_path(dbname)
    ...
    def _get_store_path(dbname: str) -> Path:
        ...
        git_path = XVaultStore._get_git_folder_path()
